...
/**
 * The rig debug viewport (spec 035/046, restored by 066): a sandbox tab that
 * shows the same controllable unit from two orthographic angles at once -- a world-aligned
 ...
 * sandbox mover (spec 066) and the movement sandbox's tuning panel, so the exact
 * rig can be driven, tuned, slowed right down and inspected -- which is how the
 * cloth was tuned in the first place. Entirely cosmetic/renderer-only: it reads
 * mover state and each rig's debug snapshot and decides nothing.
 */
 ...
 * the unit and eases its ortho zoom. Reads mover state only.
 */
class DebugScene {
  ...
  /**
   * Pose the rig from mover state (`dt` is sim time this frame, so slow-mo slows
   * the rig -- and the cloth -- too) and return the formatted numeric readout for
   * whichever unit is active.
   */
  render(p: MoverState, dt: number): string {
    const ry = -p.facing;
...
export function mountDebug(container: HTMLElement): ViewHandle {
  ...
  // This tab's ground is a bare, edgeless grid with nothing standing on it, so
  // the mover gets a world with nothing in it to match: no wall the unit could
  // stick to that you cannot see.
  ...
  const stepOnce = (): void => {
    ...
    // Cosmetic hop (spec 046): never enters the mover's input.
    if (input.takeJump()) {
      scene.robeUnit?.jump();

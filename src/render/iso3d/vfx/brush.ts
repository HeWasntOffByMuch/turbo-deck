...
export interface BrushArcParams {
  readonly id: string;
  /** How far from the origin the marks are laid. The sweep's own reach. */
  readonly radius: number;
  /** Total angle covered, centred on the effect's bearing. Radians. */
  readonly sweep: number;
  /** How many marks the sweep is made of. */
  readonly marks?: number;
  /** Each mark's length in world units. */
  readonly length?: number;
  readonly lifetimeTicks?: number;
  /** The bright end of the ramp, where a mark is freshest. */
  readonly bright?: PaletteKey;
...
/**
 * A stroke swept along an arc (spec 230): the swing.
 *
 * The painted answer to `slash_arc`, which has been in the library since spec
 * 121 and has never had a caller. Brush marks rather than stretched sparks,
 * because this is a *blade going past* and the vocabulary this game draws
 * combat in is paint (specs 158-161) -- a sweep of sparks reads as something
 * catching fire.
 *
 * ## One emitter, not one per mark
 *
 * `brushCross` next door builds an emitter per stroke, because two arms at two
 * fixed yaws is exactly two facts. This cannot: `SHAPE.arc` places a particle
 * from its index **within its own burst**, so marks spread over several spawns
 * would each restart that index and land on top of each other. One burst lays
 * them along the sweep in order, which is the property the shape exists for.
 *
 * What makes it read as a *swing* rather than as an arc appearing is the stroke
 * animation itself: every mark draws itself out along its own path (specs 158,
 * 159), and a blade covers its arc in about six ticks anyway, so a draw-on
 * staggered per mark would be describing a gesture slower than the real one.
 *
 * ## The velocity is load-bearing, and it is not there to move anything
 *
 * `brush-sweep` is yawed by its own ground track ({@link ORIENT.groundVelocity}),
 * and `SHAPE.arc` writes a **tangent** spawn direction -- so the marks follow
 * the curve, and the effect's own rotation aims the whole sweep, because
 * `system.ts` turns a spawn direction by it. That is the whole mechanism.
 *
 * Which means two numbers here are constraints rather than taste. `speed` is
 * small and **never zero**: at zero there is no ground track and every mark
 * takes the fallback bearing, which looks like a fence and is the exact failure
 * this shape was added to fix. And `gravity` is **absent**: the basis reads only
 * x and z, so a falling mark would keep its yaw while sinking out of the plane
 * it is supposed to be lying in.
 */
export function brushArc(params: BrushArcParams): EffectDefinition {
  const marks = params.marks ?? 12;
  const length = params.length ?? Math.max(6, params.radius * 0.34);
  const life = params.lifetimeTicks ?? 14;
  ...
  return {
    ...
    emitters: [
      {
        id: 'sweep',
        shape: { kind: 'arc', radius: params.radius, sweep: params.sweep },
        emission: { kind: 'burst', count: marks },
        lifetimeTicks: [life, life + 4],
        // Along the tangent, and the reason it is not zero is above. Small
        // enough that a mark travels a couple of units over its life: this is
        // the blade's own motion carrying the paint, not the paint being thrown.
        speed: [26, 44],
        // Nothing extra on top of the tangent. A spread here would turn the
        // marks away from the curve they are meant to be lying along, which is
        // the one thing the whole shape is for.
        spreadRadians: 0,
        drag: 2.2,
        size: { keys: [[0, length * 0.94], [0.3, length], [1, length * 0.96]] },
        // Opaque nearly to the end, like every mark in this file: paint leaves
        // by coming apart rather than by being turned down.
        alpha: { keys: [[0, 1], [0.68, 1], [1, 0]] },
        color: { stops: [[0, bright], [0.6, bright], [1, deep]] },
        render: 'mesh',
        mesh: { shape: 'brush-sweep' },
        blend: 'alpha',
        // Fizzle rather than retract, for `brushCross`'s reason: these marks are
        // rooted at their middles, and a retract drags a centred mark toward its
        // own tip -- so the sweep would come apart into pieces sliding off the
        // arc it was drawn on.
        strokeDecay: 'fizzle',
      },
    ],

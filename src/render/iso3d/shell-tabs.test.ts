/**
 * Which tabs a page is offered (specs 140, 254).
 *
 ...
 * or, since spec 254, in front of a player.
 */
...
describe('the tabs a page is offered', () => {
  ...
  it('offers only the game on a finger, and in the shipped client', () => {
    // One filter over both reasons (spec 254). The argument is `gameOnly`
  ...
  it('draws no tab buttons when there is only one tab to be on', () => {
    ...
    // (spec 254): the buttons are not hidden, there is nothing left to draw.

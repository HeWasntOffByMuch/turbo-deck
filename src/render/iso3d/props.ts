 ...
 *
 * A single sheet would be the honest reading of "near-zero thickness" and is
 * wrong in three separate ways at once -- it vanishes when seen from below, it
 * casts a shadow only from one side, and its rim has no edge to catch the light.
 * A closed shell 2.2 units thick against a 44-unit radius is 5% and reads as a
 * sheet, and every one of those problems goes away.
 *
...
export function speciesHeight(species: TreeSpecies): number {
  ...
  if (shape.kind === 'lobed') {
    ...
    const crown = slabLayout(shape).reduce((high, slab) => Math.max(high, slab.y + slab.rise), 0);
...
export function bareTrunkHeight(species: TreeSpecies): number {
  const shape = SPECIES[species];
  // The lowest slab's *rim*, not its plane: the underside is what you see the
  // trunk against, and it hangs `rise` below the height the slab is placed at.
  if (shape.kind === 'lobed') return shape.height * shape.canopyBase - (slabLayout(shape)[0]?.rise ?? 0);

...
export function appearanceOf(entity: AppearanceInput): Appearance {
  switch (entity.kind) {
    case EntityKind.Player:
      return { rig: 'player', typeId: 'player', radius: PLAYER_RADIUS, showsHealth: true };

    case EntityKind.Projectile: {
      // A projectile's typeId is the ability that threw it, so its size comes
      // from the same definition the server flew it with.
    ...
    case EntityKind.Prop:
      return { rig: 'prop', typeId: entity.typeId || 'prop', radius: DEFAULT_MONSTER_RADIUS, showsHealth: false };

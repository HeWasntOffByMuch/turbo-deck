...
describe('the marks a client can read', () => {
  ...
  it('scales its height with the prop, so a big sign has a big pick volume', () => {
    const [one] = signMarks([sign()]);
    const [two] = signMarks([sign({ scale: 2 })]);
    expect(one?.height).toBeCloseTo(SIGN_PLAN.postHeight + SIGN_PLAN.height, 6);
    expect(two?.height).toBeCloseTo((one?.height ?? 0) * 2, 6);
  });

  it('picks wider than it blocks', () => {
    // The collider is the post and the pick is the board: a pick at the
    // collider's radius is a six-unit stick to hit at a hundred units of camera
    // distance, and the whole feature is a click.
    const [mark] = signMarks([sign()]);
    expect(mark).toBeDefined();
    expect(pickRadius(mark as SignMark)).toBeGreaterThan((mark as SignMark).radius * 3);
  });
});

describe('which sign the cursor named', () => {
  const marks = (props: readonly Prop[]): readonly SignMark[] => signMarks(props);
  ...
  function groundOf(ray: RayLike): { x: number; y: number } {
    const hit = at(ray, -ray.origin.y / ray.direction.y);
    return { x: hit.x, y: hit.z };
  }

  it('answers the sign whose board the ray enters', () => {
    const all = marks([sign({ x: 200, y: -120 })]);
    const ray = aimAt(200, SIGN_PLAN.postHeight + SIGN_PLAN.height / 2, -120);
  ...
  it('answers a board even though the ground under the cursor is nowhere near it', () => {
    ...
    const ray = aimAt(0, SIGN_PLAN.postHeight + SIGN_PLAN.height / 2, 0);
  ...
  it('answers the nearer of two boards the same ray enters', () => {
    ...
    // nothing.
    const board = SIGN_PLAN.postHeight + SIGN_PLAN.height / 2;
    const ray = aimAt(0, board, 0);
    // Further along the same ray, at the height it has come down to by the time
    // it is near the ground -- which is where a *second* signpost's column can
    // still be crossed.
    const beyond = at(ray, BACK + (board - 10) / -ray.direction.y);
  ...
  it('falls back to the footprint for a click that landed by the post', () => {
    ...
    // beside a signpost is.
    const ray = aimAt(500, 0, 500);
    expect(pickSign(ray, all, { x: 505, y: 495 })?.key).toBe(all[0]?.key);
  });
...
describe('the index a pointer asks sixty times a second', () => {
  it('walks the store only when the store has moved on', () => {
    ...
    index.update(7, props);
    index.update(7, props);
    index.update(7, props);
    expect(walks).toBe(1);
    index.update(8, props);
    expect(walks).toBe(2);
  });
  ...
  it('rebuilds on any revision change, not only a bigger one', () => {
    ...
    const index = new SignIndex();
    index.update(9, () => [sign({ x: 0 })]);
    index.update(1, () => [sign({ x: 300 })]);
  ...
  it('finds a sign by key, and answers null for one that has gone', () => {
    ...
    const index = new SignIndex();
    const [mark] = index.update(1, () => [sign({ x: 120, y: 40 })]);

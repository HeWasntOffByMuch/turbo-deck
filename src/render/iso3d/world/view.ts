...
export async function mountWorld(container: HTMLElement): Promise<ViewHandle> {
  ...
  function updateLoading(view: ReturnType<typeof client.view>): void {
    ...
    // The light pool (spec 248), on the same terms: what each slot is actually
    ...
    const meshState =
      ...
      `:${aurasDrawn}:${lightsHeld}:${lightsOffered}`;
    if (meshState !== lastMeshState) {
      ...
      root.dataset['worldLights'] = `lit=${String(lightsHeld)} offered=${String(lightsOffered)}`;
      root.dataset['nav'] =

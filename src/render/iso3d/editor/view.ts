...
export function mountEditor(container: HTMLElement): ViewHandle {
  ...
  const frame = (time: number): void => {
    ...
    if (input.takePaintEnd()) {
      ...
      // A repaint owes the document a revision and nothing above it: walkability
      // is ground, solidity and the water line, a prop's colour comes from its
      // own part rather than from what it stands on, and a marker sits at a
      // height. None of the three moved.

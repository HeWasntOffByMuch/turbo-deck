/**
 * The bundle gate's own arithmetic (specs 199, 200).
 *
 ...
 * which since spec 200 is a manifest and 224 regions rather than one file, so
...
describe('the bundle gate', () => {
  ...
  it('fails a build that emitted the manifest and no regions', () => {
    // The failure spec 200's split introduced, and the reason the floor is a
  ...
  it('would have failed the world it is meant to pass, measured the old way', () => {
    // Spec 199 asked for the largest single asset, which was right for one

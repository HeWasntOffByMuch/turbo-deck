...
/**
 * Entry point for the poker-combo tension prototype (spec 014). Wires the DOM
 * HUD + Canvas2D arena to the deterministic combo sim through a fixed-timestep
 * loop. The loop is the only place real elapsed time becomes sim ticks; every
 * gameplay decision lives in the sim/cards/game layers below.
 */
...
function main(): void {
  ...
  const arena = new ArenaView(canvas);
  ...
  const frame = (time: number): void => {
    ...
    while (accumulator >= TICK_MS) {
      ...
      const result = stepComboGame(state, input.sample(playerScreen));
      state = result.state;

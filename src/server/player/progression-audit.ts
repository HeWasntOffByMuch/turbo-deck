...
import { preparedApplies } from '../sim/abilities.js';
...
export const TRAIT_GATES: readonly TraitGate[] = [
  ...
  {
    // Through `preparedApplies` rather than restating its condition (spec 270
    // extracted it, spec 271 gates on it). A gate that re-spelled the predicate
    // would be a second copy of it, free to stop agreeing with the consumer --
    // which is the exact failure this whole table exists to catch.
    trait: 'preparedWindupScale',
    gate: 'an ability a prepared cast applies to (`preparedApplies`)',
    satisfiedBy: () => ALL_ABILITIES.filter((a) => preparedApplies(a)).map((a) => a.id),
  },
  {
    // Spec 270's Weaving, gated here by spec 271's merge. Two rows because the
    // mechanic has two halves and either could be orphaned on its own: something
    // has to *build* a stack, and something has to be *acted on* by one.
    trait: 'weaveMaxStacks',
    gate: 'an ability that is not a basic attack, to build a stack on',
    satisfiedBy: () => ALL_ABILITIES.filter((a) => a.basicAttack !== true).map((a) => a.id),
  },
  {
    trait: 'weaveEffectPct',
    gate: 'an ability that applies an affliction, which is what the stacks scale',
    satisfiedBy: () =>
      ALL_ABILITIES.filter((a) =>
        (a.effects ?? []).some((effect) => effect.kind === 'applyDot'),
      ).map((a) => a.id),
  },
  {

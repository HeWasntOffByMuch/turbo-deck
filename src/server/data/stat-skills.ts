...
import { SCALING, STAT_SKILL_THRESHOLDS } from './scaling.js';

export interface StatSkillDefinition {
  readonly id: string;
  readonly attribute: AttributeKey;
  readonly name: string;
  /** Attribute value needed before a point may go in. */
  readonly requires: number;
  /** Which of {@link STAT_SKILL_THRESHOLDS} this sits on: 1, 2 or 3. */
...
const [T1, T2, T3] = STAT_SKILL_THRESHOLDS as [number, number, number];
...
const DEFINITIONS: readonly StatSkillDefinition[] = [
  // ======================= STRENGTH =======================
...
export const ALL_STAT_SKILLS: readonly StatSkillDefinition[] = DEFINITIONS;

export const STAT_SKILLS: ReadonlyMap<string, StatSkillDefinition> = new Map(
  DEFINITIONS.map((definition) => [definition.id, definition]),
);

export function statSkillById(id: string): StatSkillDefinition | null {
  return STAT_SKILLS.get(id) ?? null;
}

/** Every stat skill for one attribute, in tier then id order. */
export function statSkillsFor(attribute: AttributeKey): readonly StatSkillDefinition[] {

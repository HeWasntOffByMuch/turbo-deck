...
describe('two players in one world', () => {
  ...
  it('fight the same monster, and both see it take the damage', async () => {
    ...
    // Find a monster to fight, and put both players on it.
    //
    // **Teleported rather than walked**, and the reason is that the walk was
    // never the claim -- the comment it replaces said so in as many words
    // ("getting to that body is not the claim") and then spent 1,800 ticks and a
    // whole-world nav grid on it. The northern-mountain pass is what made that
    // bill come due: it moved every hostile camp away from the village, so the
    // nearest fightable body went from a few paces to 1,054 units across a
    // rebuilt square, and the route out of it runs within 0.04 units of a hut's
    // collider. `slideCircle` refuses a step into a circle rather than sliding
    // off one it is already touching, so the walk wedged there and never
    // arrived: measured, 204 units of progress in thirty seconds of sim.
    //
    // `server.teleport` is the authoritative path -- the same one the admin
    // console drives and the one `probe-walk-back.ts` uses to cross the map --
    // so it does stick, which is what the note this replaces was really about:
    // *a hand-written record* does not survive a tick, because the tick mirrors
    // authoritative positions back over it. This is not a hand-written record.
    //
    // Both of them, because the claim is that two replicas agree: interest is
    // per player, so a monster only Ana can see is a test that compares her
    // against nothing.
    let monsterId = -1;
    for (let i = 0; i < 600 && monsterId < 0; i++) {
      ...
      if (!target) continue;
      // Beside it rather than on it: two bodies on one point are pushed apart by
      // `resolveCrowding`, and a player shoved out of reach on the first tick is
      // the walk problem again in miniature.
      server.teleport('ana', target.x + 30, target.y);
      server.teleport('ben', target.x - 30, target.y);
      // Both replicas have to have caught up before the fight is the subject.
      for (let settle = 0; settle < 30; settle++) await tick();
      if (seen(ana, target.id) && seen(ben, target.id)) monsterId = target.id;
    }
    ...
    for (let i = 0; i < 1800 && !landed; i++) {
      ...
      const before = it.health;
      if (away > 40) {
        // It wandered, or a blow sent it running -- a grazer flees when it is
        // hit (spec 163). Straight at it is enough now that the two started
        // beside each other; there is no village to route around.
        const reach = away || 1;
        ana.sendInput({ moveX: dx / reach, moveY: dy / reach, facing: Math.atan2(dy, dx), buttons: 0 });
      } else {

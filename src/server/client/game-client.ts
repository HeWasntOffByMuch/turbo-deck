...
/**
 * The wire index Flow rides under (spec 253), looked up rather than typed.
 *
...
export interface ClientView {
   ...
   * it may not yet walk out of (spec 253).
   *
...
export class GameClient {
  ...
  sendInput(intent: Omit<PredictedInput, 'seq'>): { readonly x: number; readonly y: number } | null {
    ...
    //
    // Unless the follow-through is still committed (spec 253), in which case the
    ...
    //
    // A committed follow-through is the third (spec 253), and it is the one the
  ...
  cancelCast(): void {
    ...
    //
    // Unless the follow-through is still committed (spec 253), where the server
  ...
  /**
   * Flow stacks this body is visibly carrying, off the replica (spec 253).
   *
  ...
  /**
   * Whether a follow-through is holding this body right now (spec 253).
   *

 ...
 *     and only from the **cancel point** onward (spec 253), because a
    ...
    return resolveAttackTiming(
      {
        ...
        // The same threshold a basic attack gets (spec 253). A follow-through is
  ...
  return resolveAttackTiming(
    {
      ...
      // reaches the attack point and, since spec 253, the tick a follow-through
      ...
      // **The authored length, whole** (spec 253). Agility used to divide this
 ...
 * (spec 253): the character's own threshold, less what Flow is taking off it.
 *
...
/**
 * The tick this cast may first be voluntarily walked out of (spec 253).
 *
...
export function cancelCast(entity: ServerEntity, tick: number, reason: number): CancelResult {
  ...
  if (cast.committed) {
    // **The cancel point** (spec 253). A follow-through is a phase with two

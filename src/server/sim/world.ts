...
export interface StepContext {
  ...
  /**
   * Every enemy spawn point the map places (spec 073). Empty for a world with
...
export interface SpawnSpec {
  ...
  /**
   * Who this body starts out fighting, if anyone (spec 073).
   *
...
export function step(
  ...
  for (const entity of [...working.values()]) {
    ...
    // Everything else leaves nothing behind (spec 073). A five-second body that
...
function monsterIntent(
  ...
  // Nothing initiates (spec 073). A monster's only route to a target is the
...
/**
 * Refills the map's spawn points (spec 073).
 *

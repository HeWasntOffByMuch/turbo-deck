...
export function step(
  ...
  for (const entity of state.entities.values()) {
    ...
    // dropping the movement components of anything still casting, which is
    // exactly what a refusal means.
    //
...
export function advanceProgression(
  ...
  return { ...entity, statuses, stillSinceTick, stanceSinceTick, shield, poise, health };
}
...
export function advanceRest(entity: ServerEntity, tick: number, resting: boolean): ServerEntity {
  ...
  // **Second Wind re-arms here** (spec 239), beside the flask charge and for

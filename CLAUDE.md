...
| `npx tsx scripts/preview-lance.ts` | What the Warden's beam looks like on the arena's real ground (spec 259), in both phases, with a player standing in it and one beside it. Rasterised in software for `preview-aim.ts`'s reason -- what is being judged is a *shape* -- and it prints the two numbers a thumbnail hides: what fraction of the frame the beam paints, and how far it moves the ground's colour **in retro colour bands**, which is the unit that decides whether a mark survives the quantize at all |

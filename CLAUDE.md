...
| `npm run verify` | **The inner loop** (spec 274): `typecheck:fast && lint:fast && test:changed`, which is the three gates above over warm caches and only the tests a change reaches. 14s for a typical commit against the cold three's 3 minutes (19s + 42s + 2m03s), because the median commit here touches **one** TypeScript file. Measured warm: a first run after a full `npm test` re-checks more, because the suite's own compile invalidates the `.tsbuildinfo`. It is not the merge gate and does not try to be -- CI still runs all three cold and every test, since a gate that only ran what it thought was affected would be trusting the dependency graph about the one thing nobody is watching |
...
| `npm run audit:progression` | Every specialization tier at every attribute value it can be bought at, and whether the purchase reaches anything the sim reads (specs 241, 244). `--all` lists the working ones too. Since spec 272 it also runs a **conditional-effect observation pass**: the tier audit proves a purchase *moves a trait*, which Steady Aim did in all twelve of its cells while being incapable of firing, so each gated mechanic carries its own scenario and is driven through a real fight. `NOT OBSERVED` means the fight written to trigger it did not -- never that a short generic fight missed a rare effect, which is the false positive that would make the pass noise. Complementary to spec 271's static reachability rather than a rival: that one asks whether any *content* can satisfy a gate, this asks whether the *simulation* ever does, and Steady Aim's was satisfiable by content and unsatisfiable by the tick order |

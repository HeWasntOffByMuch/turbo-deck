# 088 — Attack speed is a delay, and a shot's speed is its own
...
3. **A shot's speed was tied to that same stat** (spec 087), which made sense
...
same constant so the reach is still exactly what the table describes (spec 087's
...
- **Reach is still the table's**, for every projectile row, and still at least
  the ability's own range — the assertion spec 087 added, now with the shooter
  out of it.

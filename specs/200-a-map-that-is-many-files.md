...
- **Nothing can be loaded lazily**, which is what spec 202's bounded residency
  needs underneath it.
...
(spec 198's `MAP_CHUNK_REQUEST_RADIUS` of 2) drags in at worst alignment.
...
`spawnPointsFrom` walks every chunk today, and spec 202 needs the spawner list
...
- Lazy loading itself. This spec makes the map *loadable* lazily and keeps
  reading it whole; spec 202 is what stops reading it whole.

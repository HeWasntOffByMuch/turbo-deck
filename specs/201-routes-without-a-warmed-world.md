# 201 — Routes without a warmed world
...
- **Terrain residency.** This makes nav local to where players are; the *map*
  is still loaded whole. Spec 202 is what stops that, and it consumes this.
...
- **Eviction policy beyond residency.** A tile is dropped when no active chunk
  needs it. Ageing an idle tile is spec 202's kind of question.

# 203 — A boot that does not mesh the world
...
- **Heap** at 12,960 chunks measures **0.26 GB**, not the 2.0 GB the plan
  projected. That projection was made before spec 200 took `nav` out of the map
...
- **Entity eviction, `SpawnerState`'s third state, and the "eviction is not
  death" table.** Spec 202 stopped the tick *walking* what nobody is near;
...
- **`parse`**, which is now the largest single boot cost at the target (~1.5 s)
  and is spec 200's region files doing exactly what they were built to make

# 206 — A tick that costs what is near you

## Problem

The sim already knows what is resident: `isSimulated` gates stepping on
`activeChunks`, and spec 202 narrowed that to 7×7 interest chunks. But three
...
`radius`, through `circlesInRect` (added in spec 205 for nav tiles). A bounding
...
- **Terrain residency.** The map is still loaded whole; boot and heap are
  untouched. That is spec 207, and it is where the 48 s and 2.0 GB live.

...
| **Cancel point** | The tick within the backswing from which it may be broken off (spec 253, `backswingCancelTicks`). Before it the body is committed; the length of the backswing itself is not something progression moves. |
...
Spec 253 sharpened the same rule into a second one: **name the thing the number

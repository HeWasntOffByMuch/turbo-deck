...
Status: **every phase is done — specs 201 through 210 are written and
...
not claimed here. It is exactly what spec 201's nav benchmark exists to produce.
...
### Phase 0 — a map you can measure (spec 201)
...
### Phase 1 — a zoom you choose (spec 202) — **done**
...
### Phase 2 — the map leaves the bundle (spec 203) — **done**
...
### Phase 3 — a map that is many files (spec 204)
...
### Phase 4 — routes without a warmed world (spec 205)
...
### Phase 5 — a tick that costs what is near you (spec 206)
...
assertions spec 201 wrote asserting the *hole* are inverted, as their own note
...
### Phase 6 — a boot that does not mesh the world (spec 207)
...
that projection predates spec 204 taking `nav` out of the format, an array per
...
### Phase 7 — a client that forgets behind it (spec 208)
...
### Phase 8 — growing without rewriting (spec 209)

**Done.** Spec 204 made a grow *write* only what it touched; it still opened the

...
(spec 154). Everything else here is a rule or a direction. **A future direction
...
*Implemented, in one narrow form:* delayed rarity reveal (spec 154). A drop
...
This is the reason spec 154's `common` tier has a reveal delay of exactly zero
...
**Not yet implemented, and explicitly out of scope for spec 154.** There is no
...
**Current rule.** Presentation must never obstruct responsive play. Spec 154
...
- **Implemented:** rarity as a property of an item row, a per-monster drop
  ...
  clock, and a restrained tier-shaped presentation over it — all of spec 154 and
  nothing beyond it.

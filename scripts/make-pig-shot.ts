/**
 * Writes the pig's authored bow shot into its clip library (spec 164).
 *
 *   npx tsx scripts/make-pig-shot.ts
 *
 * The same shape as `make-pig-strike.ts`, and for the same reasons: the work is
 * pure and lives in `src/units/`, this file is only the part that cannot be, and
 * the `.glb` it writes is committed so the animation reviews as a diff of
 * `src/units/pig-shot.ts` rather than as a blob of bytes.
 *
...
import { PIG_SHOT, SHOT_CLIP_ID } from '../src/units/pig-shot.js';
...
const OUT = join(CLIP_DIR, `${SHOT_CLIP_ID}.glb`);

const GENERATOR = 'turbo-deck authored clip (spec 164)';
...
function main(): void {
  ...
  for (const key of PIG_SHOT.keys as readonly PoseKey[]) {
  ...
  const document = authorClipDocument(PIG_SHOT, rig, GENERATOR);
  ...
  console.log(
    `  ${PIG_SHOT.durationMs}ms at ${PIG_SHOT.fps}fps -- ${frameCount(PIG_SHOT)} frames over ` +
      `${bones.length} bones, ${PIG_SHOT.keys.length} key poses`,
  );
  console.log(`  ${bones.join(' ')}`);
  console.log('\n  next: npm run bake:units, then npx tsx scripts/preview-shot.ts');
}

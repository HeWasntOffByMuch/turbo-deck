...
import { checkBundle, MAX_JS_BYTES, MIN_MAP_BYTES, type Emitted } from '../src/render/bundle-budget.js';
...
function main(): void {
  ...
  console.log(
    `emitted map         ${(report.mapBytes / 1048576).toFixed(2)} MB  ` +
      `(floor ${(MIN_MAP_BYTES / 1048576).toFixed(2)} MB)`,
  );
